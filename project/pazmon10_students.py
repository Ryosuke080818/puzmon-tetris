...
def draw_field(
    ...
    # フィールド全体のサイズを計算
    total_w = cols * (SLOT_W + SLOT_PAD) - SLOT_PAD
    total_h = rows * (SLOT_W + SLOT_PAD) - SLOT_PAD
    
    # 白枠
    padding = 7  # フィールドと枠の間のスペース
    frame_rect = pg.Rect(
        LEFT_MARGIN - padding,        
        FIELD_Y - padding,              
        total_w + (padding * 2),    
    ...
    pg.draw.rect(screen, (255, 255, 0), frame_rect, width=2, border_radius=3)
...
def draw_top(screen, enemy, party, font, enemy_frames, show_frame=0):
    ...
    if enemy["max_hp"] > 0:
        ...
        screen.blit(name, (525, WIN_H-702))
        pg.draw.rect(screen, (255, 255, 255), (523, WIN_H-662, 424, 22))
        enemy_bar = hp_bar_surf(enemy['hp'], enemy['max_hp'], 420, 18)
        screen.blit(enemy_bar, (525, WIN_H-660))
        ...
        screen.blit(enemy_hp_text, (838, WIN_H-702))
        ...
        screen.blit(label, (525, WIN_H-612))
        pg.draw.rect(screen, (255, 255, 255), (523, WIN_H-572, 424, 22))
        party_bar = hp_bar_surf(party['hp'], party['max_hp'], 420, 18)
        screen.blit(party_bar, (525, WIN_H-570))
        ...
        screen.blit(party_hp_text, (838, WIN_H-612))

def draw_message(screen, text, font, msg_frame_img):
    frame_x, frame_y = 495, WIN_H - 520
    screen.blit(msg_frame_img, (frame_x, frame_y))
    lines = text.split('\n')
    for i, line in enumerate(lines):
        surf = font.render(line, True, (230,230,230))
        screen.blit(surf,(525,WIN_H-495+i*40))
...
def main():
    ...
    msg_frame_img = pg.image.load("frame.png").convert_alpha()
    msg_frame_img = pg.transform.smoothscale(msg_frame_img, (470, 250))
    ...
    while running:
            ...
            if wait_timer == 0:
                ...
                for (r, c, L, direction) in current_runs:
                    ...
                    if elem == "命":
                        ...
                        if dmg > 0:
                            enemy['hp'] -= int(dmg)
                            summary[elem] += int(dmg)
                            attack.play()
                    
                    field[r][c] = "無"
                    
                for elem, value in summary.items():
                    if value > 0:
                        if elem == "命":
                            current_messages.append(f"HP +{value}")
                            party['hp'] = min(party['max_hp'], party['hp'] + value)
                            party['last_damage_time'] = current_time
                            party['hp_event_type'] = 'heal'
                        else:
                            current_messages.append(f"{elem}攻撃！ {value}ダメージ")
                            enemy['hp'] -= value
                            enemy['enemy_damage_time'] = current_time
        ...
        draw_message(screen, message, font, msg_frame_img)
        for eff in active_effects:
            eff.draw(screen)

        # 「タイトルへ」ボタン
        if enemy["name"] != "宝箱" and party["hp"] > 0:
            mx, my = pg.mouse.get_pos()
            back_hover = back_btn.collidepoint(mx, my)
            pg.draw.rect(screen, (80, 80, 110) if back_hover else (55, 55, 75), back_btn, border_radius=10)
            pg.draw.rect(screen, (230, 230, 230), back_btn, width=2, border_radius=10)
